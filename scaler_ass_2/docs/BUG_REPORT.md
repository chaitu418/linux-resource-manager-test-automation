...
Steps to Reproduce:
Place a process in the CRITICAL class.
Set the base memory limit to 8 GB.
Trigger the 2x memory increase logic (expected limit: 16 GB).
Observe the process termination by OOM killer due to the 8 GB cgroup cap.
...
Steps to Reproduce:
Set the system-wide process limit for a user to a low value (e.g., 1024).
Use the Resource Manager to launch a critical process under that user and set its individual ulimit -u to a higher value (e.g., 4096).
Attempt to spawn more than 1024 threads/processes under the critical process.
